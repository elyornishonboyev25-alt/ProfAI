...
async function main() {
  const directory = await mkdtemp(join(tmpdir(), 'profai-premium-ui-'))
  const dom = new JSDOM('<div id="root"></div>', { url: 'http://localhost:5173', pretendToBeVisual: true })
  try {
    for (const key of ['window', 'document', 'HTMLElement', 'Element', 'Node', 'Event', 'SVGElement', 'getComputedStyle']) {
    ...
    await build({
      entryPoints: ['scripts/tests/premium-ui.tsx'], bundle: true, platform: 'node', format: 'cjs',

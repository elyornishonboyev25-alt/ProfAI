...
async function main() {
  const directory = await mkdtemp(join(tmpdir(), 'profai-sat-mistakes-ui-'))
  const dom = new JSDOM('<div id="root"></div>', { url: 'http://localhost:5173', pretendToBeVisual: true })
  try {
    // This jsdom version lacks MathML style objects used by the question renderer.
    const createElementNS = dom.window.document.createElementNS.bind(dom.window.document)
    dom.window.document.createElementNS = (...args) => {
      const element = createElementNS(...args)
      if (args[0] === 'http://www.w3.org/1998/Math/MathML' && !element.style) {
        Object.defineProperty(element, 'style', { value: dom.window.document.createElement('span').style })
      }
      return element
    }

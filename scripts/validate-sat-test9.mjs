...
const { getSATTest, getSATSectionTest, isSATTestComplete, satAvailabilityNote } = await load('src/features/sat/catalog.ts')
...
assert.equal(isSATTestComplete(test), false)
assert.match(satAvailabilityNote(test), /Math Module 2 is unavailable/)
...
assert.equal(math.questionCount, 22)
assert.equal(math.totalDurationSeconds, 35 * 60)
assert.equal(isSATTestComplete(math), false)
assert.equal(isSATTestComplete(getSATTest(8)), true)
const attempt = createSATAttempt(test.id, test.modules, 'exam')
assert.equal(attempt.moduleDeadlines.__test__ - attempt.startedAt, 99 * 60 * 1000)
const answers = Object.fromEntries(questions.map((q) => [q.id, q.correctAnswer]))
assert.equal(scoreSATModules(test.modules, answers).correct, 76)
assert.equal(scoreSATModules(test.modules, {}).unanswered, 76)
...
console.log('SAT Test 9 valid: 76 source answers, restored Q10, corrected Q7, visuals, timing, scoring, and partial-section metadata.')

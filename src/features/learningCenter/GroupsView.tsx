...
function MiniMetric({ label, value, tone = 'default' }: { label: string; value: string | number; tone?: 'default' | 'good' | 'bad' }) {
  return <div className={`rounded-xl p-3 text-center ${tone === 'good' ? 'bg-emerald-50' : tone === 'bad' ? 'bg-red-50' : 'bg-slate-50'}`}><p className={`text-lg font-bold ${tone === 'good' ? 'text-emerald-700' : tone === 'bad' ? 'text-red-700' : 'text-slate-950'}`}>{value}</p><p className="mt-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-400">{label}</p></div>
}
...
function CreateGroupModal({ open, onClose, slug, teachers, onDone }: { open: boolean; onClose: () => void; slug: string; teachers: Array<{ userId: string; user: { fullName: string } }>; onDone: () => void }) {
  ...
  return <Modal open={open} onClose={onClose} busy={busy} title="Create a learning group" description="Define the cohort target and assign accountable teaching ownership."><form onSubmit={submit} className="space-y-4"><label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-700">Group name</span><input required minLength={2} value={name} onChange={(event) => setName(event.target.value)} className={inputClass} placeholder="SAT Advanced — Group A" /></label><div className="grid gap-4 sm:grid-cols-2"><label><span className="mb-1.5 block text-xs font-bold text-slate-700">Exam track</span><select value={examTrack} onChange={(event) => setExamTrack(event.target.value as CenterExamTrack)} className={inputClass}><option value="SAT">SAT</option><option value="IELTS">IELTS</option><option value="BOTH">Both</option></select></label><label><span className="mb-1.5 block text-xs font-bold text-slate-700">Target score</span><input value={targetScore} onChange={(event) => setTargetScore(event.target.value)} className={inputClass} placeholder={examTrack === 'SAT' ? '1450' : '7.5'} /></label></div><label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-700">Lead teacher</span><select value={teacherId} onChange={(event) => setTeacherId(event.target.value)} className={inputClass}><option value="">Assign later</option>{teachers.map((teacher) => <option key={teacher.userId} value={teacher.userId}>{teacher.user.fullName}</option>)}</select></label><label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-700">Schedule</span><input value={schedule} onChange={(event) => setSchedule(event.target.value)} className={inputClass} placeholder="Mon · Wed · Fri, 16:00" /></label>{error ? <p className="rounded-xl bg-red-50 px-3 py-2 text-xs font-bold text-red-700">{error}</p> : null}<div className="flex justify-end gap-2"><button type="button" disabled={busy} onClick={onClose} className={secondaryButton}>Cancel</button><button disabled={busy} className={primaryButton}><Plus className="h-4 w-4" />{busy ? 'Creating...' : 'Create group'}</button></div></form></Modal>
}

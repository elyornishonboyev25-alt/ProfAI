...
export default function StudentDetailView({ slug, studentId, canManage }: { slug: string; studentId: string; canManage: boolean }) {
  ...
  async function analyze() {
    setAnalyzing(true); setActionError('')
    try { setInsight((await learningCenterApi.analyzeStudent(slug, studentId)).insight) }
    catch (failure) { setActionError(failure instanceof Error ? failure.message : 'Could not refresh analysis. Please try again.') }
  ...
  return (
    <div className="space-y-6">
      ...
      <button type="button" onClick={() => navigate(`/learning-center/${slug}/students`)} className="inline-flex items-center gap-2 text-xs font-bold text-slate-500 transition hover:text-blue-700"><ArrowLeft className="h-4 w-4" /> Back to students</button>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(20rem,.8fr)]">
        ...
        <CenterPanel className={`p-5 sm:p-6 ${activeInsight.tone === 'warning' ? 'ring-1 ring-red-100' : ''}`}>
          <div className="flex items-center justify-between"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/20"><BrainCircuit className="h-5 w-5" /></span><button type="button" disabled={analyzing} onClick={() => void analyze()} className="inline-flex items-center gap-1.5 rounded-xl border border-blue-100 bg-blue-50 px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-blue-700 hover:bg-blue-100 disabled:opacity-60"><RefreshCw className={`h-3.5 w-3.5 ${analyzing ? 'animate-spin' : ''}`} />{analyzing ? 'Analyzing' : 'Refresh AI'}</button></div>
          <p className="mt-5 text-[10px] font-bold uppercase tracking-[.2em] text-blue-600">ProfAI performance analysis</p>
          <h2 className="mt-1 text-xl font-bold tracking-[-.03em] text-slate-950">{activeInsight.headline}</h2>
          <p className="mt-3 text-sm font-medium leading-6 text-slate-600">{activeInsight.summary}</p>
          <div className="mt-5 space-y-2">{activeInsight.priorities.map((priority, index) => <div key={priority} className="flex gap-3 rounded-xl bg-slate-50 px-3.5 py-3"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-lg bg-blue-600 text-[10px] font-bold text-white">{index + 1}</span><p className="text-xs font-bold leading-5 text-slate-700">{priority}</p></div>)}</div>
        </CenterPanel>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
        ...
        <CenterPanel className="p-5 sm:p-6">
          <div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-blue-600">Private coaching log</p><h2 className="mt-1 text-xl font-bold text-slate-950">Teacher notes</h2></div>
...
function Assignments({ assignments, onOpen }: { assignments: StudentDetailAssignments; onOpen: (route: string, assignmentId: string) => void }) {
  return <CenterPanel className="p-5 sm:p-6"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-blue-600">Learning plan</p><h2 className="mt-1 text-xl font-bold text-slate-950">Assignments</h2></div><div className="mt-4 space-y-3">{assignments.length ? assignments.slice(0, 6).map((item) => <article key={item.id} className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50/70 p-3.5"><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${item.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-700' : item.status === 'OVERDUE' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}>{item.status === 'COMPLETED' ? <CheckCircle2 className="h-5 w-5" /> : <CalendarClock className="h-5 w-5" />}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-slate-900">{item.title}</p><p className="mt-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-400">{item.status.replace('_', ' ')} · due {new Date(item.dueAt).toLocaleDateString()}</p></div>{item.status !== 'COMPLETED' ? <button type="button" onClick={() => onOpen(item.routePath, item.assignmentId)} className={secondaryButton}>Open</button> : null}</article>) : <p className="py-8 text-center text-sm font-semibold text-slate-400">No assignments have been issued.</p>}</div></CenterPanel>
}

function RecentResults({ results }: { results: LearningResult[] }) {
  return <CenterPanel className="p-5 sm:p-6"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-blue-600">Assessment history</p><h2 className="mt-1 text-xl font-bold text-slate-950">Recent results</h2></div><div className="mt-4 divide-y divide-slate-100">{results.length ? results.slice(0, 8).map((result) => <div key={`${result.source}-${result.id}`} className="flex items-center gap-3 py-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-blue-50 text-blue-600"><Sparkles className="h-4 w-4" /></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-slate-900">{result.title}</p><p className="mt-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-400">{result.skill.replace(/_/g, ' ')} · {new Date(result.completedAt).toLocaleDateString()}</p></div><div className="text-right"><strong className="text-lg font-bold text-slate-950">{result.score}</strong><p className="text-[10px] font-bold text-slate-400">/ {result.maxScore}</p></div><span className="hidden text-[10px] font-semibold text-slate-400 sm:flex"><Clock3 className="mr-1 h-3.5 w-3.5" />{Math.round(result.durationSec / 60)}m</span></div>) : <p className="py-8 text-center text-sm font-semibold text-slate-400">No test results yet.</p>}</div></CenterPanel>
}

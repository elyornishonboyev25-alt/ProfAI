import { november2025IntlQuestions } from '@/data/sat/november2025IntlQuestions'
...
function mathSkill(prompt: string): [string, string] {
  ...
  if (/percent|probability|mean|median|data set|conversion|ratio|frequency table|histogram/.test(text)) return ['Problem-Solving and Data Analysis', 'Data, Ratios & Probability']
...
function explanationFor(question: (typeof november2025IntlQuestions)[number]) {
...
function buildQuestion(question: (typeof november2025IntlQuestions)[number]): SATQuestion {
  ...
  const isSplitSource = question.id === 'rw1-9' || question.id === 'rw2-12'
  ...
  return {
    ...
    asset: `/sat/november-2025-intl/questions/${question.id}.jpg`,
    assetWidth: 1920,
    assetHeight: isSplitSource ? 2048 : 1024,
    explanation: explanationFor(question),
  }
}

export const SAT_NOVEMBER_2025_INTL_MODULES: SATModule[] = MODULE_META.map((module) => ({
  ...module,
  questions: november2025IntlQuestions
    .filter((question) => question.moduleId === module.id)
    .sort((first, second) => first.number - second.number)
    .map(buildQuestion),
}))

export const SAT_NOVEMBER_2025_INTL = {
  id: 'november-2025-intl-v1' as const,
  title: 'Digital SAT Practice Test 5',
  subtitle: 'November 2025 International · Version 1',
  questionCount: 98,
  totalDurationSeconds: SAT_NOVEMBER_2025_INTL_MODULES.reduce(

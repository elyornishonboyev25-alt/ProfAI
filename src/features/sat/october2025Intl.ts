import { october2025IntlQuestions } from '@/data/sat/october2025IntlQuestions'
...
function mathSkill(prompt: string): [string, string] {
  const text = prompt.toLowerCase()
  if (/circle|triangle|angle|cylinder|surface area/.test(text)) return ['Geometry and Trigonometry', 'Geometry & Trigonometry']
  if (/percent|sample|mean|median|range|scatterplot|population|rate in miles/.test(text)) return ['Problem-Solving and Data Analysis', 'Data, Ratios & Probability']
  if (/quadratic|exponential|minimum|maximum|radical|square root|\^2/.test(text)) return ['Advanced Math', 'Nonlinear Functions']
  return ['Algebra', 'Linear Equations & Functions']
}

function explanationFor(question: (typeof october2025IntlQuestions)[number]) {
  const answerText = question.kind === 'multiple-choice'
    ? question.choices.find((choice) => choice.key === question.correctAnswer)?.text
    : question.correctAnswer

  return `**Step 1:** Identify the quantities, evidence, or language relationship being tested.
...
function buildQuestion(question: (typeof october2025IntlQuestions)[number]): SATQuestion {
  const section = sectionFor(question.moduleId)
  const [domain, skill] = section === 'reading-writing' ? readingSkill(question.prompt) : mathSkill(question.prompt)
  return {
    ...question,
    section,
    domain,
...
export const SAT_OCTOBER_2025_INTL_MODULES: SATModule[] = MODULE_META.map((module) => ({
  ...module,
  questions: october2025IntlQuestions
    .filter((question) => question.moduleId === module.id)
    .sort((first, second) => first.number - second.number)
    .map(buildQuestion),
}))

export const SAT_OCTOBER_2025_INTL = {
  id: 'october-2025-intl-v1' as const,
  title: 'Digital SAT Practice Test 6',
  subtitle: 'October 2025 International · Version 1',
  questionCount: 98,
  totalDurationSeconds: SAT_OCTOBER_2025_INTL_MODULES.reduce((total, module) => total + module.durationSeconds, 0),

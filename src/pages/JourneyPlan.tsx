...
export default function JourneyPlan() {
  ...
  const loadPlan = useCallback(async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'Your study plan could not be loaded.')
      setStatus('error')
    }
  }, [])
  ...
  if (status === 'loading') {
    return <div className="grid min-h-[70vh] place-items-center px-4"><div className="text-center"><div className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-red-600 text-white shadow-xl"><Route className="h-7 w-7" /></div><Loader2 className="mx-auto mt-5 h-5 w-5 animate-spin text-red-600" /><p className="mt-3 text-sm font-bold text-slate-500">Preparing your study plan…</p></div></div>
  }
    ...
    <div className="workspace-page relative min-h-screen px-3 pb-24 pt-3 sm:px-5 sm:pt-5 lg:px-5 lg:pb-6">
      <div className="mx-auto max-w-[92rem]">
        <header className="mb-5 flex flex-wrap items-center justify-between gap-3 px-1">
          <div><p className="text-[10px] font-black uppercase tracking-[.18em] text-red-700">Personal university workspace</p><h1 className="text-2xl font-black tracking-[-.04em] text-slate-950 sm:text-3xl">Study Plan</h1></div>
